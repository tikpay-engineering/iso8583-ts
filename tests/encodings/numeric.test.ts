...
describe('numeric', () => {
  ...
  describe('encodeNumeric', () => {
    it('returns ascii encoded Buffer when encoding is ascii', () => {
      ...
      const ret = encodeNumeric({ kind: Kind.Numeric, length: 10, encoding: NumericEncoding.ASCII }, '12345678')
    ...
    describe('bcd encoding', () => {
      it('appends 0 if length is odd', () => {
        ...
        encodeNumeric({ kind: Kind.Numeric, length: 5, encoding: NumericEncoding.BCD }, '12345')
  ...
  describe('decodeNumeric', () => {
    it('returns decoded ascii value when encoding is ascii', () => {
      const ret = decodeNumeric(
        { kind: Kind.Numeric, length: 4, encoding: NumericEncoding.ASCII },
        Buffer.from('123456', 'ascii'),
        0,
      )
    ...
    it('returns decoded BCD value when encoding is BCD', () => {
      ...
      const ret = decodeNumeric({ kind: Kind.Numeric, length: 4, encoding: NumericEncoding.BCD }, buf, 0)
    ...
    it('uses ceil for odd digit lengths (bcd)', () => {
      ...
      const ret = decodeNumeric({ kind: Kind.Numeric, length: 3, encoding: NumericEncoding.BCD }, buf, 0)
