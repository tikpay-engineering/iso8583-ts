...
describe('numeric', () => {
  ...
  describe('encodeNumeric', () => {
    it('returns ascii encoded Buffer when encoding is ascii', () => {
      ...
      const ret = encodeNumeric({ kind: 'n', length: 10, encoding: 'ascii' }, '12345678')
  ...
  describe('decodeNumeric', () => {
    it('returns decoded ascii value when encoding is ascii', () => {
      const ret = decodeNumeric({ kind: 'n', length: 4, encoding: 'ascii' }, Buffer.from('123456', 'ascii'), 0)
    ...
    it('returns decoded BCD value when encoding is BCD', () => {
      ...
      const ret = decodeNumeric({ kind: 'n', length: 4, encoding: 'bcd' }, buf, 0)
    ...
    it('defaults to bcd when encoding is not given', () => {
      ...
      decodeNumeric({ kind: 'n', length: 4 }, buf, 0)
    ...
    it('uses ceil for odd digit lengths (bcd)', () => {
      ...
      const ret = decodeNumeric({ kind: 'n', length: 3, encoding: 'bcd' }, buf, 0)
